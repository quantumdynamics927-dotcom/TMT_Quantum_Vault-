...
class ArchitectureBenchmarkRunner:
    ...
    def _get_agent_metrics(self, agent_name: str) -> Optional[AgentMetrics]:
    ...
    def run_benchmark(self, mode: str) -> ArchitectureBenchmark:
        ...
        if agents:
            ...
            benchmark.fitness_std = statistics.stdev(fitnesses) if len(fitnesses) > 1 else 0.0
            ...
            benchmark.resonance_std = statistics.stdev(resonances) if len(resonances) > 1 else 0.0
        ...
        if mode == "core13":
            ...
            benchmark.platonic_solid_alignment = self._calculate_platonic_alignment(agents)
        else:
            benchmark.provenance_completeness = self._calculate_provenance_completeness(agents)
            benchmark.workflow_efficiency = self._calculate_workflow_efficiency(agents)
            benchmark.governance_compliance = self._calculate_governance_compliance(agents)
    ...
    def _calculate_platonic_alignment(self, agents: List[AgentMetrics]) -> float:
        ...
        phi_balance = 1.0 - (statistics.stdev(inner_phis) / statistics.mean(inner_phis)) if inner_phis else 0.0
    ...
    def compare_modes(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "comparison": {
                ...
                "fitness_stability_improvement": core13.fitness_std - extended17.fitness_std,
