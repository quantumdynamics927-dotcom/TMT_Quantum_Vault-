...
class CoordinationMetricsCollector:
    ...
    def record_contract_completion(self, contract: AgentContract) -> None:
        ...
        self._agent_processing_time[output.agent_name] += (
    ...
    def get_metrics(self) -> CoordinationMetrics:
        ...
        contradiction_rate = (
            contradiction_count / total_outputs
            if total_outputs > 0 else 0.0
        )

        # Calculate delegation metrics
        delegations = self._delegation_window.get_all()
        successful_delegations = sum(
            1 for d in delegations if d.get("success", False)
        )
        delegation_success_rate = (
            successful_delegations / len(delegations)
        ...
        successful_recoveries = sum(
            1 for r in recoveries if r.get("success", False)
        )
        ...
        if resonances and tasks:
            ...
            success_rate = (
                sum(1 for t in tasks if t.get("success", False)) / len(tasks)
            )
        ...
        phi_alignment_rate = (
            phi_aligned / len(resonances) if resonances else 0.0
        )
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "resonance_fitness_correlation": (
                metrics.resonance_fitness_correlation
            ),
...
class CoordinationAnalyzer:
    ...
    def analyze_trends(
        self, history: list[CoordinationMetrics]
    ) -> dict[str, Any]:
    ...
    def identify_bottlenecks(self) -> list[dict[str, Any]]:
        ...
        if (
            metrics.delegation_count > 0 and
            metrics.delegation_success_rate < 0.8
        ):
            bottlenecks.append({
        ...
        if (
            metrics.recovery_attempts > 0 and
            metrics.recovery_success_rate < 0.7
        ):
            bottlenecks.append({
...
class MetricsExporter:
    ...
    def export_trace(
        ...
        trace_data = {
            ...
            "final_status": (
                trace.final_status.value if trace.final_status else None
            ),
            ...
            "contracts": [
                {
                    ...
                    "output_status": (
                        c.output.status.value if c.output else None
                    ),
                    "output_confidence": (
                        c.output.confidence if c.output else None
                    ),
                }
